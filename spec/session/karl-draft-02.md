...
**Status:** DRAFT v1
**Based on:** richard-revision-01.md (implementation spec), BRIEFING.md (shared agreements)
...
1. **Classification (under 1 second).** JORDAN reads your question. It checks it against a list of known-dangerous patterns (like "delete all files"). If nothing dangerous matches, and the question looks simple, it takes the FAST path.

2. **Quick Answer (a few seconds).** JORDAN generates an answer directly. No planning, no research, no safety checks beyond the initial pattern scan. Before showing you the answer, it runs one final scan on the output itself -- making sure the answer does not contain anything dangerous, any garbled internal formatting, or any refusal to answer. If the answer passes, JORDAN adds a small disclaimer: "This is a quick answer generated without research or tool access. For complex topics requiring deeper analysis, I can run a full investigation."

3. **Done.** You see the answer. No further steps.
...
**What can go wrong:** The answer may be incomplete or shallow because no research was done. The disclaimer warns you about this. If your question accidentally triggered the pattern scanner (e.g., you asked "how does the `rm -rf` command work?" rather than "run `rm -rf`"), JORDAN escalates to the STANDARD path instead -- slightly slower, but safer.
...
7. **Branching Factor Check (under 1 second).** JORDAN checks the plan's shape. If the plan fans out like an exploding tree (too many parallel sub-tasks, or a chain too deep), it may halt and request a simpler plan structure. This is a mathematical check, not an AI judgment.
...
9. **Approval Gate (waiting for you).** If ANY sub-task is rated HIGH or CRITICAL, JORDAN stops and shows you a briefing. The briefing has two tiers:
   - **Summary (always shown):** The goal, the overall risk level, and any critical flags.
...
13. **Replan (if needed).** If the evaluation was not SUCCESS, JORDAN creates a delta plan -- a modified version of only the sub-tasks that failed. It runs a quick structural check on the delta, re-checks risk for changed parts, and re-executes. This loop can repeat up to 3 times total. If after 3 replans the result still is not SUCCESS, JORDAN escalates to you with a summary of what it tried and what went wrong.
...
4. **Expanded Persona Set.** The pre-mortem uses all available personas, including the Resource Analyst (checks for memory limits, API rate limits, timeout constraints) and the Devil's Advocate (challenges the plan's unstated assumptions).
...
JORDAN uses multiple personas (different AI roles with different perspectives) to generate failure scenarios:
...
- **Resource Analyst (DEEP path):** "Here are the memory, storage, API, and timeout constraints that will kill this plan."
...
Each persona's accuracy is tracked over time. If a persona cries wolf too often (more than 50% false alarms), it is disabled until reviewed.
...
The backbrief can reject a plan up to 2 times. After that, it force-accepts the plan and moves on, but the approval briefing notes that structural verification was incomplete.

### Branching Factor Monitor: "Stopping when the work explodes"

Some plans look reasonable on paper but would explode into thousands of parallel operations if executed. For example: "Check every file in the repository for security issues" -- if the repository has 10,000 files, checking each one in parallel would overwhelm the system.

The branching factor monitor checks two things:

1. **Width (branching factor):** How many sub-tasks run in parallel at any given moment. If any point in the plan has sub-tasks that each spawn more sub-tasks (branching factor of 1 or more), the plan is halted. This is a mathematical property of the plan's shape -- think of it as detecting a pyramid scheme before anyone loses money.

2. **Depth:** How long the chain of sequential dependencies is. A chain of 50 steps where each depends on the previous one is not branching, but it is still problematic (any single failure breaks the entire chain). Maximum depth defaults to 10.

There is also a runtime monitor that tracks whether any sub-task, during execution, dynamically creates new sub-tasks (something the plan did not anticipate). If the total number of sub-tasks (original + spawned) exceeds twice the original count, execution halts and replanning is triggered.
...
[+] Branching factor: 0.3 -- within limits
```
...
**During replanning:** If JORDAN's replans make things WORSE (the evaluation goes from PARTIAL to FAILURE), it detects this "degrading replan" pattern and escalates to you directly after 2 consecutive degradations.
...
| "This output was not evaluated against any criteria." | The plan had no defined success criteria, so JORDAN cannot confirm quality. | The output may be fine -- it just was not checked. For important tasks, consider rephrasing your request with clearer expectations. |
...
[ F.4 APPEND    [ F.5 REPLACE WITH ]
 DISCLAIMER ]    SAFE FALLBACK ]
    |              |
    v              v
[ F.6 SYNTHESIZE (pass-through) ]
    |
    v
[ F.7 EVALUATE (always SUCCESS) ]
    |
    v
...
(plan_version = 1 or incremented on regeneration)
       |
       v
       |
  ╔══════════════════════════════════════╗
...
Static DAG analysis:
 - Branching factor (b >= 1 -> halt)
 - Max depth (> max_depth -> halt)
       |
       v
...
[HALT -> escalate     |
 ...
 structural flag]     |
    ...
    v
[ S.13 RISK FUSION ]
Reconcile 4 risk signals per sub-task:
 final_risk = max(
...
All personas active:
 ...
 - Optimist
 - Devil's Advocate
 - Resource Analyst (NEW: memory, storage, API, timeout checks)
       |
       v
...
Sub-task spawn ceiling  original × 2    Runtime halt (trigger REPLAN)
...
*End of JORDAN v2 ELI5 Flowchart Document -- DRAFT v1.*
