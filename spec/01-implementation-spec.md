# JORDAN v2 Final Implementation Specification

**Status:** Final (incorporating arbitration of all 29 findings)
**Date:** 2026-05-25
**Based on:** BRIEFING.md (agreements, prohibitions, resolutions), richard-revision-01.md (implementation revision), ARBITRATION.md (29 findings resolved, 9 design decisions)

---

## How to Read This Document

This is the final implementation specification for the JORDAN v2 pipeline. It supersedes all previous drafts and incorporates every arbitration decision from ARBITRATION.md. All resolution markers, critique markers, design-decision markers, and arbitration-required markers have been resolved and removed. This document is internally consistent.
...
LangGraph Subgraphs chosen over private-dict workaround. Each sub-task executes as an independent LangGraph Subgraph with its own reduced state schema and independent checkpointing. The parent EXECUTE node uses `Send()` to fan out to Subgraphs and a `Join` node to collect results. Rationale: true checkpoint isolation is the only way to guarantee the parallel safety claim. The private-dict workaround loses state on interrupt/resume, which breaks the safety invariant. Full specification below in section 10.

**Critical Issue 2: FRAGO Loop Validation Gap**
The FRAGO replan loop (REPLAN -> RISK -> EXECUTE -> SYNTHESIZE -> EVALUATE) includes a lightweight three-step validation on every delta plan: (a) structural consistency check (DAG cycle/broken-reference check -- algorithmic, sub-millisecond), (b) risk delta check (re-classify changed sub-tasks only, which RISK ASSESSMENT already does), (c) branching factor check on the delta's DAG. These three checks replace the full BACKBRIEF/PREMORTEM/BRANCHING-MONITOR re-run. The full re-run is skipped for performance, but structural integrity is guaranteed. Full specification in section 13.7.

**Critical Issue 3: Unreconciled Risk Signals**
A risk fusion step is introduced between PREMORTEM and APPROVAL. It reconciles risk signals from CLASSIFY complexity_factors, RISK ASSESSMENT risk_levels, and PREMORTEM failure_scenario severities. Branching violations are handled by the BRANCHING MONITOR -> REPLAN path and are excluded from risk fusion. This produces a single reconciled risk level per sub-task presented in the APPROVAL briefing, with the individual assessments retained for drill-down. Full specification in section 9.10.
...
Performs TAPE-inspired triage at pipeline entry to determine which of three execution paths (FAST, STANDARD, DEEP) the incoming request follows. Conservative classifier bias: any uncertainty escalates UP. This node is the sole gatekeeper for pipeline path selection.

Classification produces one of three outcome labels:
- **TRIVIAL** routes to FAST path (simple queries, known-safe patterns).
- **MODERATE** routes to STANDARD path (planning, research, pre-mortem).
- **COMPLEX** routes to DEEP path (full pipeline with multi-COA, MAKER decomposition, sequential execution, always-on approval).
...
Same as v1, with classification labels noted above.

### 1.5 EDGE CASES

**Edge case 1.5.A -- Denylist false-positive feedback loop:**
...
**Edge case 1.5.B -- Denylist update mechanism:**
...
**Edge case 1.5.C -- Denylist disagreement with LLM classifier logged and fed back:**
...
Lightweight execution path for trivial queries. v2 adds a **lightweight output guardrail** before passing results to SYNTHESIZE, closing the acknowledged bypass from v1.
...
**Edge case 2.5.A -- Post-hoc output guardrail for FAST path:**
...
**Edge case 2.5.B -- Tool-call hallucination definition across model APIs:**
...
**Edge case 2.5.C -- Factual disclaimer appended to all FAST responses:**
All FAST path responses have a disclaimer appended: "This is a quick answer generated without research or tool access. For complex topics requiring verification or deeper analysis, I can run a full investigation." The disclaimer is configurable (can be shortened or removed per environment).

**Edge case 2.5.D -- Safety refusal differentiated from uncertainty:**
...
The output guardrail is a mandatory step before SYNTHESIZE on the FAST path.
...
| `n_coas` | `int` | Graph state (set by CLASSIFY) | Number of courses of action to generate. Default 1 for STANDARD, 3 for DEEP path. |
...
On the DEEP path, PLAN generates `list[Plan]` internally and selects the best one. The selected `Plan` is written to state. The full list is preserved in `state.all_coas` for audit trail.

### 3.5 EDGE CASES

**Edge case 3.5.A -- `pipeline_phase` is a concrete state field:**
...
**Edge case 3.5.B -- Plan versioning and checksum:**
The `Plan` dataclass includes a `plan_version: int` field (incremented on every regeneration) and `plan_checksum: str` (SHA-256 of canonical JSON encoding of plan content). PREMORTEM stores failure_scenarios keyed by `plan_checksum`. When a new plan arrives (plan_checksum differs from previous), PREMORTEM archives old scenarios and starts fresh. When `plan_checksum` is identical (trivially regenerated), PREMORTEM reuses existing scenarios. This prevents scenario accumulation across different plan versions.

**Edge case 3.5.C -- Skill library feedback on rejected templates:**
When a plan seeded from a skill library template is rejected by BACKBRIEF or triggers a fatal PREMORTEM finding, the template's `failure_count` is incremented and its `last_failure_reason` is updated in the skill library. If a template accumulates `max_failures_before_deprioritization` (configurable, default 3) failures from BACKBRIEF or PREMORTEM, its `priority_boost` is set to `-1.0` (effectively deprioritized -- only used if no other template matches).

**Edge case 3.5.D -- DEEP path multi-COA planning (DD-004):**
When `path == "DEEP"`, PLAN receives `n_coas=3`. It generates 3 distinct plans (Courses of Action). These 3 plans are submitted to BACKBRIEF for structural validation. The structurally soundest plan is selected based on: DAG validity, coupling scores, resource contention minimization, and closest alignment with Commander's Intent. If multiple plans pass BACKBRIEF, the one with the lowest coupling score and resource contention is selected. If no plan passes BACKBRIEF, the closest-to-passing plan is regenerated with BACKBRIEF's flags addressed (following standard BACKBRIEF revision flow). The selected plan then proceeds to PREMORTEM. This is DEEP path only; STANDARD path always uses `n_coas=1`.

**Edge case 3.5.E -- MAKER decomposition for correctness-critical sub-tasks (DD-004):**
On the DEEP path, after plan selection, sub-tasks flagged with both `correctness_critical=True` AND `verifiable_output=True` are decomposed into atomic MAKER steps (m=1). Each atomic step represents a minimal verifiable unit of work with an associated verification checkpoint. The decomposition is algorithmic: split the sub-task description into sequential steps, each with its own verification criterion. The verification checkpoint can be: a structured test (for code), a validation check (for data), or a review step (for document output). The decomposed sub-tasks replace the original in the plan's sub_task list. This increases sub-task count but ensures correctness-critical work has built-in verification. DEEP path only.
...
class Plan:
    ...
    plan_version: int          # Incremented on every regeneration
    plan_checksum: str         # SHA-256 of canonical JSON content
    metadata: dict

@dataclass
class PipelineState:
    ...
    n_coas: int = 1            # Number of COAs to generate (3 for DEEP)
    all_coas: Optional[list[Plan]] = None  # Full COA list for audit (DEEP only)
```

### 3.7 INTERACTIONS

- "Re-entry from BACKBRIEF" is "Regeneration triggered by BACKBRIEF flags".
- "Re-entry from PREMORTEM" is "Regeneration triggered by PREMORTEM fatal flags".
- Graph-level assertion: `assert state.replan_count == 0, "PLAN cannot be re-entered during FRAGO replan loop"`.
...
Key change: BACKBRIEF's revision counter (`backbrief_revision_count`) is now **independent** from PREMORTEM's cycle counter (`premortem_cycle_count`). Each has a max of 2 independent iterations. The combined worst case is 4 iterations (2 BACKBRIEF + 2 PREMORTEM), but structural soundness is never sacrificed for cycle counting convenience.

BACKBRIEF runs after PLAN (which is before RESEARCH) as a structural check on the plan's DAG, dependency structure, and hidden coupling.

On the DEEP path, BACKBRIEF also validates all 3 generated COAs for structural soundness, contributing to COA selection (see section 3.5.D).
...
Revision ceiling for independent counter:
...
**Edge case 4.5.A -- `backbrief_forced` flag has a defined consumer:**
`plan.metadata["backbrief_forced"] = True` is consumed by:
1. **APPROVAL GATE:** The structured briefing includes a banner: "Plan structurally forced through backbrief ceiling -- DAG may contain unresolved issues."
...
**Edge case 4.5.B -- Delta-aware re-verification:**
...
**Edge case 4.5.C -- DSM heuristics defined (pseudocode):**
The DSM hidden-coupling detection algorithm:
...
This algorithm is encoded in `_analyze_dsm()` and operates on the sub-task definitions directly.
...
- **On DEEP multi-COA:** BACKBRIEF validates all 3 generated plans. The best structurally-sound plan is selected.
...
RESEARCH validates cache results against the current tool registry before returning them.
...
| Field | Type | Source | Description |
|-------|------|--------|-------------|
| `tool_registry` | `ToolRegistry` | Cross-cutting | Validates that cached tool_recommendations reference currently available tools. |
...
**Edge case 5.5.A -- Tool-availability validation on cache results:**
...
**Edge case 5.5.B -- Early escalation for significant knowledge gaps:**
...
**Edge case 5.5.C -- Stale-but-incorrect cache detection (not just TTL):**
...
**Edge case 5.5.D -- User scope change during interruption triggers CLASSIFY re-run (K-H3):**
If the pipeline pauses at RESEARCH for a pre-execution gap escalation (5.5.B) and the user responds with information that changes the scope of the request, CLASSIFY must be re-run on the updated scope before resuming. Scope change is detected by computing a diff vector between the original `user_message` and the user's interruption response. If the diff indicates new domains, new tool requirements, or expanded complexity (measured by keyword overlap < 50% or introduction of > 2 new domain tags), the pipeline routes back to CLASSIFY. The re-classification result may change the path (e.g., STANDARD -> DEEP). The previous RESEARCH results are preserved in the audit trail.
...
Updated with pre-execution gap escalation: if early escalation is triggered (5.5.B), the graph pauses at RESEARCH with an interrupt, waits for user input, and resumes from RESEARCH with the user-provided context. If scope change is detected during the interruption, the pipeline routes back to CLASSIFY.
...
| Field | Type | Source | Description |
|-------|------|--------|-------------|
| `knowledge_gaps` | `list[KnowledgeGap]` | RESEARCH (or RESEARCH cache snapshot on replan) | On replan, RESEARCH is re-queried for cache lookup only (no early escalation, no LLM context gathering). This provides current knowledge_gaps without the overhead of full RESEARCH re-execution. |
...
**Edge case 6.5.A -- Changed sub-task diff algorithm defined by decision table:**
...
**Edge case 6.5.B -- Minimum batch coherence for classification:**
...
**Edge case 6.5.C -- On replan, knowledge_gaps are refreshed:**
RESEARCH is NOT fully re-executed on replan, but RESEARCH's cache query IS re-executed. This means `knowledge_gaps` on replan reflects any new cache entries that were written between the first pass and the replan. This is a lightweight SQL query -- no LLM calls, no early escalation.
...
PREMORTEM is a mandatory gate. If fatal flaws are found (severity HIGH+ and likelihood > 50%), the pipeline routes back to PLAN for regeneration, then BACKBRIEF re-runs. Max 2 pre-mortem cycles.
...
**Persona set (DD-009):** PREMORTEM uses the following personas to generate diverse failure scenarios:
- **Standard personas (all non-FAST paths):**
  1. Pessimist -- identifies everything that could go wrong
  2. Optimist -- identifies optimistic assumptions that could fail
  3. Devil's Advocate -- challenges core assumptions and design decisions
  4. Resource Analyst -- identifies resource constraints (memory, storage, API rate limits, timeout constraints, data volume)
- **DEEP-only persona:**
  5. Domain Expert -- provides domain-specific failure modes (DEEP path only, adds subject-matter expertise)
- **Total:** 5 personas (4 standard, 1 DEEP-only)
...
| Field | Type | Source | Description |
|-------|------|--------|-------------|
| `plan_checksum` | `str` | PLAN | SHA-256 hash of plan content. Used to detect plan changes between cycles. |
...
**Edge case 7.5.A -- Plan-versioned scenario storage:**
PREMORTEM stores `failure_scenarios` keyed by `plan_checksum`. When a new plan arrives with a different checksum, old scenarios are **archived** (not appended). The archive records: `plan_version`, `plan_checksum`, `scenarios`, `timestamp`, `cause_of_regeneration` (which persona found what). The APPROVAL briefing can display the revision history if desired.

**Edge case 7.5.B -- Reconciliation with RISK ASSESSMENT:**
Per the risk fusion design (section 9.10), PREMORTEM's `failure_scenarios` severities are reconciled with RISK ASSESSMENT's `risk_levels` in a risk fusion step before APPROVAL. The exact mechanism: `final_risk[sub_task] = max(RISK_ASSESSMENT.risk_levels[sub_task], max(PREMORTEM.scenarios_for[sub_task].severity, default=LOW))`.

**Edge case 7.5.C -- Persona calibration tracking:**
...
**Edge case 7.5.D -- Plan revision quality tracking:**
...
- If a revision does NOT address the flags, the next PREMORTEM cycle will re-flag them. This is expected behavior -- the cycle counter prevents infinite oscillation.

**Edge case 7.5.E -- PREMORTEM's fatal flag priority over BACKBRIEF's structural pass:**
...
**Edge case 7.5.F -- PREMORTEM force-pass on all-persona failure (K-H1):**
If ALL persona LLM calls fail (return no scenarios due to system error, not due to "no flaws found"), PREMORTEM cannot produce scenarios and is force-passed with a critical log alert. This is a system-health failure, not an advisory opt-out. The force-pass is logged and APPROVAL is informed: "Pre-mortem gate was unable to analyze this plan due to a system error."

**Edge case 7.5.G -- Ceiling-hit force-pass with advisory findings (K-H1):**
When `premortem_cycle_count >= 2` and PREMORTEM still finds fatal flaws, the plan is force-passed with the highest-severity findings attached as advisory (not blocking). The APPROVAL briefing includes: "Pre-mortem ceiling reached -- plan accepted with the following unaddressed findings." The advisory findings are included in the Tier 1 summary with a warning banner.
...
class PremortemNode:
    def analyze(self, state: PipelineState) -> PipelineState:
        ...
        Scenarios are archived per plan_checksum,
...
2. **True runtime sub-task spawn monitor (during execution):** Tracks dynamic sub-task spawning during EXECUTE. If a sub-task dynamically spawns new sub-tasks (via tool calls that return new tasks), the runtime monitor detects this and can halt execution mid-flow.
...
| `max_depth` | `int` | Configuration | Maximum allowed DAG depth before halting. Default 10. |
...
| `spawned_sub_task_count` | `int` | EXECUTE | Count of dynamically spawned sub-tasks during execution |
| `max_total_sub_tasks` | `int` | Configuration | Maximum total sub-tasks before runtime halt. Default = `original * 2`. |
...
| `runtime_monitor_active` | `bool` | EXECUTE | Whether the runtime spawn monitor should track this execution |

### 8.5 EDGE CASES

**Edge case 8.5.A -- Depth-based check is a first-class feature:**
The depth-based check (`max_depth > 10` triggers warning, `max_depth > max_allowed_depth` triggers halt) is implemented as a first-class check alongside branching factor. The check produces its own entry in the branching report: `depth_violation: bool`, `depth_violation_threshold: int`.

**Edge case 8.5.B -- 45% rule disentangled from branching factor:**
...
**Edge case 8.5.C -- Runtime monitor behavior on replan:**
...
Key changes:
1. **Risk fusion step added** (section 9.10) -- reconciles risk signals before presentation.
...
4. **DEEP path override** -- when `path == "DEEP"`, ALL sub-tasks require human approval regardless of risk level. The standard risk-based routing does not apply.
...
| Field | Type | Source | Description |
|-------|------|--------|-------------|
| `scope_change_message` | `Optional[str]` | Graph state | Human-readable message describing scope change since last approval (for replan re-entry with scope changes). |
| `has_unaddressed_safety_flags` | `bool` | PREMORTEM | Whether the pre-mortem or backbrief identified safety-related issues that remain unaddressed. |
...
**Edge case 9.5.A -- Progressive disclosure briefing format:**
...
- DEEP path notice: if `path == "DEEP"`, a banner states "DEEP path: all sub-tasks require human approval"
...
**Edge case 9.5.B -- Briefing diff for replan cycles:**
...
**Edge case 9.5.C -- Rename "halt" to "mandatory approval" for CRITICAL:**
Throughout the spec and code, "pipeline halt" for CRITICAL has been renamed to "mandatory human approval." The pipeline does not stop permanently -- it pauses at APPROVAL GATE and waits for the user. The user can approve (proceed with CRITICAL sub-task) or reject. CRITICAL sub-tasks always route through APPROVAL, but `approval_decision = "approved"` allows execution to proceed.

**Edge case 9.5.D -- DEEP path always-on approval (DD-004):**
When `path == "DEEP"`, ALL sub-tasks require human approval regardless of their individual risk levels. The standard risk-based routing (CRITICAL -> mandatory approval, HIGH/MEDIUM/LOW -> auto-approve) does not apply. The APPROVAL briefing for DEEP path includes: a multi-COA summary (all plans considered, why this plan was selected), MAKER decomposition details (which sub-tasks were decomposed, verification checkpoints), and explicit sequential execution notice. This is DEEP path only.
...
- **Updated trigger chain:** BRANCHING FACTOR MONITOR -> RISK FUSION (section 9.10) -> APPROVAL GATE.
...
- **DEEP override:** If `state.path == "DEEP"`, the routing check always routes to APPROVAL regardless of risk levels.
...
Reconciles risk signals produced by different nodes into a single, coherent risk assessment presented to the human at APPROVAL. Without this step, the APPROVAL briefing shows contradictory signals (e.g., RISK ASSESSMENT says LOW, PREMORTEM says HIGH) with no resolution.
...
4. If knowledge_gaps with severity HIGH affect this sub-task:
   - Elevate base to at least HIGH (missing information increases risk).
5. Final output: dict of sub_task_id -> reconciled RiskLevel.
```

**Note:** Branching violations are handled by the BRANCHING MONITOR -> REPLAN path. Risk fusion does not process stop signals. This avoids dead code referencing graph states that cannot occur at this point in the topology (DD-008).

**FRAGO staleness guard (DD-006):** PREMORTEM data is plan-version-scoped. In FRAGO mode, risk fusion excludes PREMORTEM data via version check -- only fresh RISK ASSESSMENT re-classification of changed sub-tasks is used. This prevents stale pre-mortem data from influencing risk decisions on delta plans where the pre-mortem analysis was performed on a different plan version.
...
On the DEEP path, the DAG scheduler operates in `ExecutionMode.SEQUENTIAL` by default (DD-004). Sub-tasks execute one at a time respecting dependency order. Parallel execution requires explicit user opt-in during APPROVAL.
...
- **Enum:** `ExecutionMode` -- `PARALLEL` (default) or `SEQUENTIAL` (DEEP path default)
...
**Edge case 10.5.A -- Cascade prevention in compensation handler ladder:**
...
- This single-direction escalation prevents cascade.
...
**Edge case 10.5.B -- DAG scheduler specified as a first-class component:**
...
class DagScheduler:
    ...
    def max_concurrency(self) -> int:
        """Maximum parallel sub-tasks based on DAG structure and execution mode.
        Returns 1 if execution_mode == "sequential"."""
        ...
```

The DAG scheduler is the component that enforces execution ordering, detects runtime cycles (defense-in-depth after BACKBRIEF), and respects isolation_keys for resource-level sequentialization. It supports `ExecutionMode.SEQUENTIAL` from day one (DD-007).

**Edge case 10.5.C -- Atomicity requirements for sub-task execution:**
...
- These atomicity requirements are enforced by the tool wrappers in the tool registry, not by individual sub-task implementations.

**Edge case 10.5.D -- ExecutionMode.SEQUENTIAL for DEEP path (DD-004):**
The DAG scheduler supports an `execution_mode` parameter with two values: `PARALLEL` (default for STANDARD/FAST) and `SEQUENTIAL`. When `path == "DEEP"`, the scheduler operates in `SEQUENTIAL` mode by default: sub-tasks execute one at a time, respecting dependency order. Parallel execution on DEEP path requires explicit user opt-in, presented during APPROVAL. STANDARD and FAST paths always use `PARALLEL` mode (respecting concurrency limits and isolation keys). The `execution_mode` is exposed in the APPROVAL briefing so the user can request parallel execution if they choose.
...
- The parent graph uses `Send()` to launch Subgraphs in parallel (respecting concurrency_limit, isolation_keys, and execution_mode).
...
**Integration risk:** The hardest thing about EXECUTE is now the **Subgraph orchestration** -- specifically:
...
Second-hardest: **compensation handler cascade prevention** -- the monotonic escalation invariant (section 10.5.A) must be enforced programmatically, not just documented.
...
QualityReport on FAST path:

| Field | Value on FAST Path |
|-------|-------------------|
| `is_consistent` | `None` (not assessed) |
| `completeness_score` | `None` (not assessed) |
| `quality_review_performed` | `False` |

### 11.5 EDGE CASES

**Edge case 11.5.A -- Merge algorithm defined by output type:**
...
**Edge case 11.5.B -- Heterogeneous output format handling:**
...
**Edge case 12.5.A -- Algorithmic fallback produces PARTIAL/FAILURE distinction:**
...
This matches the LLM-based evaluation's three-state output, so the evaluation outcome is no longer a function of whether the LLM is available.

**Edge case 12.5.B -- LLM nondeterminism acknowledged with evaluation caching:**
...
**Edge case 12.5.D -- Delta-aware evaluation on replan:**
...
```python
EvaluationResult = Literal["SUCCESS", "PARTIAL", "FAILURE", "UNEQUIVALENT"]  # UNEQUIVALENT added
...
class EvaluateNode:
    def __init__(
        ...
        self._evaluation_cache: dict[str, Evaluation] = {}  # evaluation cache
    ...
    def _evaluate_algorithmic(
        ...
        Now produces PARTIAL (not just binary SUCCESS/FAILURE).
        """
        ...
```
...
UNEQUIVALENT routes like SUCCESS (output proceeds to user, annotated).

### 12.8 FAST PATH

"EVALUATION is always SUCCESS for FAST path, with `evaluation_method = 'algorithmic'` and `evaluation_detail = 'FAST path -- no criteria evaluated'."
...
5. **FRAGO risk fusion excludes pre-mortem data** by plan-version check (DD-006).
...
| Field | Type | Source | Description |
|-------|------|--------|-------------|
| `current_knowledge_gaps` | `list[KnowledgeGap]` | RESEARCH (cache re-query) | Current knowledge gaps at replan time. Re-queried from RESEARCH cache (not full re-execute). |
| `backbrief_revision_count` | `int` | Graph state | Current BACKBRIEF revision count (for info -- REPLAN does not use this for its own ceiling). |
...
**Edge case 13.5.A -- FRAGO loop includes lightweight structural validation:**
...
**FRAGO risk fusion staleness (DD-006):** On the FRAGO path, risk fusion excludes PREMORTEM data via plan-version check. The PREMORTEM analysis was performed on a different plan version (the original pre-delta plan), so its failure scenarios are not valid for the delta. Only fresh RISK ASSESSMENT re-classification of changed sub-tasks is used in the FRAGO risk fusion. Full PREMORTEM re-run only happens on full replan (non-delta), not on FRAGO.

**Edge case 13.5.B -- Compensation ladder escalation counts vs. FRAGO replan counts -- clarified:**
...
**Edge case 13.5.C -- REPLAN's delta makes the plan WORSE -- oscillation handling:**
...
**Edge case 13.5.D -- BACKBRIEF is skipped on replan BUT structural validation runs (per 13.5.A):**
On replan, BACKBRIEF, PREMORTEM, and BRANCHING MONITOR are not re-run in full. Instead, the lightweight 3-check validation (13.5.A) ensures structural integrity. PREMORTEM's persona-based failure analysis is intentionally NOT re-run on delta plans because: (a) the delta is typically small (1-3 sub-tasks), not enough to warrant full persona analysis, (b) the original PREMORTEM already analyzed the plan's core structure, (c) the risk fusion step (section 9.10) excludes stale pre-mortem data via version check. If the delta introduces a fundamentally new failure mode, EXECUTE's compensation handler can catch it, and the FRAGO loop will handle it.
...
RESEARCH cache re-query is added to the FRAGO loop (not full RESEARCH re-execution -- just a cache query).

Risk fusion in FRAGO mode: PREMORTEM data excluded by plan-version check (DD-006). Only fresh RA re-classification used.
...
7. `skill_template_sanitized` field added to template schema.
...
Updated with tenant ID filtering. Template schema includes `skill_template_sanitized: bool` indicating whether the template has been sanitized for safety (inputs validated, tool calls verified, outputs constrained). Seed templates default to `True`; newly learned templates default to `False` until reviewed.

### 14.5 EDGE CASES

**Edge case 14.5.A -- Template decay function updated:**
...
This function:
- Rewards frequency (`sqrt(use_count + 1)` grows sub-linearly).
...
**Edge case 14.5.B -- Administrative interface specified:**
...
**Edge case 14.5.C -- Template staleness check on retrieval:**
...
**Edge case 14.5.F -- Write-ahead queue for concurrent write contention:**
...
- **Crash safety:** On pipeline crash, in-flight queue entries are lost. This is acceptable: the writes are post-execution archival data (templates, failure patterns, audit logs).
- **Monitoring:** Queue depth is exposed as a metric. If queue depth exceeds `max_depth_warning` (default 500), an alert fires.

**Edge case 14.5.G -- skill_template_sanitized field (R-H5):**
All plan templates include a `skill_template_sanitized: bool` field. This field indicates whether the template has passed a safety sanitization review (inputs validated against injection patterns, tool calls verified to reference safe operations, outputs constrained to safe formats). Seed templates ship with `skill_template_sanitized = True`. Newly learned templates are created with `skill_template_sanitized = False`. Templates with `skill_template_sanitized = False` are flagged in the APPROVAL briefing and may be deprioritized in template selection.
...
All write methods (archive_plan, record_failure_pattern, add_to_allowlist) now go through the write-ahead queue instead of direct SQLite writes.
...
Updated to include the output guardrail (section 2.5.A), factual disclaimer (section 2.5.C), and safety refusal detection (section 2.5.D) in the FAST path trace.
...
3. **Resource feasibility persona:** PREMORTEM's persona set now includes a "Resource Analyst" persona. This persona's task: "Identify resource constraints that could prevent successful execution: memory limits, storage limits, API rate limits, timeout constraints, data volume constraints."

4. **Dead-end detection labeled as proactive vs. reactive:**
...
2. **PREMORTEM-over-BACKBRIEF priority (D.2):** PREMORTEM's semantic analysis overrides BACKBRIEF's structural analysis for the purpose of triggering plan regeneration. If BACKBRIEF passes (structurally sound) and PREMORTEM finds fatal flaws (semantically unsound), the plan is regenerated. If BACKBRIEF rejects (structurally broken), the plan is regenerated regardless of PREMORTEM's state. The graph topology enforces: BACKBRIEF runs first, then PREMORTEM. PREMORTEM only runs if BACKBRIEF passes.

3. **User-intent-change handling (D.3):** When the user submits a fundamentally different request mid-pipeline:
...
4. **Learning scoping (D.4):** Templates in the skill library are scoped by:
...
5. **Observability architecture (D.5):** Metrics and tracing for production operation:
   - **Per-node latency histograms:** Track p50, p95, p99 latency for each node.
   ...
   - **Ceiling hit rate:** How often each ceiling (backbrief, premortem, replan, approval) is hit.
   ...
   - These metrics are exposed via a `/metrics` endpoint (Prometheus format).

6. **Skill library bootstrapping (D.6):** Curated seed templates for cold start:
   ...
   - These curated templates are marked as `origin = "seed"` and have `use_count` starting at 5.
...
This section documents all eight structural tensions Karl identified and their resolutions.
...
**Resolution applied in:** Section 9.10 (Risk Fusion). Reconciliation step added between PREMORTEM and APPROVAL: `final_risk[sub_task] = max(RISK_ASSESSMENT.risk_levels[sub_task], max(PREMORTEM.scenarios_for[sub_task].severity, default=LOW))`. Branching violations excluded (handled by BRANCHING MONITOR -> REPLAN).
...
| DD-013 | DEEP path always-on approval | ALL DEEP sub-tasks require human approval | Complex tasks engage full pipeline with human oversight (ARBITRATION DD-004) | 9.5.D |
| DD-014 | DEEP multi-COA planning | 3 COAs generated, best selected via BACKBRIEF + PREMORTEM | Multiple perspectives on complex tasks before committing | 3.5.D |
| DD-015 | MAKER decomposition | correctness_critical AND verifiable_output sub-tasks decomposed to atomic steps | Built-in verification for correctness-critical work | 3.5.E |
| DD-016 | DEEP sequential execution | ExecutionMode.SEQUENTIAL default on DEEP path | Safety over speed for complex/safety-critical tasks | 10.5.D |
| DD-017 | Risk fusion dead code removal | halt_flag rule removed from risk fusion | Branching violations handled by BRANCHING MONITOR -> REPLAN path (DD-008) | 9.10.3 |
| DD-018 | FRAGO risk fusion staleness | PREMORTEM data excluded by plan-version check in FRAGO mode | Only fresh RA re-classification used; stale pre-mortem data excluded (DD-006) | 9.10.3, 13.5 |
| DD-019 | Persona set | 4 standard (Pessimist, Optimist, Devil's Advocate, Resource Analyst) + 1 DEEP-only (Domain Expert) = 5 total | Standardized across all spec documents (DD-009) | 7.1 |

---

## Known Limitations

### Subgraph Checkpoint Overhead

The LangGraph Subgraph approach for isolated state is the right design choice but may introduce checkpoint overhead for plans with many parallel sub-tasks. Implementation should: (a) cap parallel sub-tasks at `concurrency_limit` (default 4), (b) batch sub-tasks into groups for sequential Subgraph phases, (c) benchmark checkpoint overhead with 10, 20, 50 parallel sub-tasks and adjust if overhead exceeds 20% of execution time.

### DSM Heuristic Completeness
